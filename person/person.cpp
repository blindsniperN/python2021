...
namespace pers_class
{
  ...
  DiceRoll Person::Seduce() {
    DiceRoll ans;
    ans.to_hit = 1 + rand() % 10 + skills_.seduction_.Roll() + cd_;
  ...
  DiceRoll Person::MakeAnArgument() {
    DiceRoll ans;
    ans.to_hit = 1 + rand() % 10 + skills_.leadership_.Roll() + cd_;
    ans.dmg = 1 + rand() % 8 + parameters_.charm_.value_;
    return ans;
  }

  DiceRoll Person::Convince() {
    DiceRoll ans;
    ans.to_hit = 1 + rand() % 10 + skills_.persuasion_.Roll() + cd_;
    ans.dmg = (1 + rand() % 6) / 2 + parameters_.charm_.value_;
    return ans;
  }

  DiceRoll Person::Deceive() {
    DiceRoll ans;
    ans.to_hit = std::max(1 + rand() % 10 +
            skills_.deception_.Roll() - deceive_to_hit_ + cd_, 0);
    ans.dmg = 1 + rand() % 12 + parameters_.intelligence_.value_;
    return ans;
  }
  ...
  DiceRoll Person::Mock() {
    DiceRoll ans;
    ans.to_hit = std::max(1 + rand() % 10 +
                           skills_.etiquette_.Roll() - mock_to_hit_ + cd_, 0);
  ...
  PersonContainer::PersonContainer(const std::string& s, const stats_library::ParameterList& p,
  ...
  void Person::applyConvinceAttacker(short dmg) { health_ += dmg; }// убеждение по атакующему

  void Person::applyDeceiveDefenser(short dmg) {
    substractHealth(health_, dmg);
  } // обмануть защищающийся

  void Person::applyDeceiveAttacker() { deceive_to_hit_ += 2; cd_ = 0;} // обмануть атакующий

  void Person::applyMockDefenser(short dmg) { substractHealth(health_, dmg); } // насмехнуться по защищаемуся
  void Person::applyMockAttacker(short dmg) {
    health_ += dmg;
    mock_to_hit_ += 1;
    cd_ = 0;
  } // насмехнуться по атакующему
  // защиты
  ...
  // рычаги
  void Person::applyLove() { cd_ = 3; }// любовь
